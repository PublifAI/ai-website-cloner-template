...
You are about to reverse-engineer and rebuild **$ARGUMENTS** as pixel-perfect clones using **plain HTML + Tailwind CSS** (no frameworks).
...
├── [page-name].html           # One HTML file per unique page
...
├── js/
│   └── main.js                # Minimal vanilla JS (menu toggle, carousel, forms)
├── images/                    # Downloaded from source site
│   ├── seo/                   # Favicons, OG images
│   └── ...
├── tailwind.config.js         # Design tokens from extraction
├── build.sh                   # tailwindcss CLI build command
└── docs/
    └── research/              # Extraction artifacts (specs, screenshots)
```
...
3. **Check for discovery output.** If `docs/research/site-map.json` exists (from running `/discover-site`), read it and use it to determine which pages to clone. If `docs/research/design-system.json` exists, use it for design tokens. If `docs/research/content/` has files, use them for page content. This avoids re-scraping what was already discovered.
4. Create the output directories if they don't exist: `output/`, `output/css/`, `output/js/`, `output/images/`, `output/images/seo/`, `docs/research/`, `docs/research/components/`, `docs/design-references/`, `scripts/`.
5. When working with multiple pages, optionally confirm whether to run them in parallel (recommended) or sequentially.
...
Every section gets a specification file in `docs/research/components/` BEFORE any builder is dispatched. This file is the contract between your extraction work and the builder agent.
...
1. Read `docs/research/site-map.json` — use its page list to determine what to clone
2. Read `docs/research/design-system.json` — use for tailwind.config.js generation
3. Read `docs/research/content/*.json` — use for page content instead of re-scraping
...
If `docs/research/design-system.json` exists, use its values directly.
...
Create spec files in `docs/research/components/` with the same template as before, but change the target file path and implementation notes:
